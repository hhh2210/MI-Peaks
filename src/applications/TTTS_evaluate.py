...
def parse_args():
    ...
    # TTTS related
    parser.add_argument("--thinking_tokens_file_path", type=str, default=None)
    parser.add_argument("--max_tokens_per_call", default=2048, type=int)
    parser.add_argument("--thinking_token", default="", type=str)
...
def batch_TTTS_generation(prompts, llm, tokenizer, args, thinking_token, stop_words):
    ...
    ### 2. Generation with thinking tokens
    budget_prompts = []

    for q, a in zip(prompts, outputs):
        concat_prompt = f'{q} {a} {thinking_token}'
    ...
    outputs_budget = []
    for output in vllm_outputs_budget:
        generated_text = output.outputs[0].text.strip()
        outputs_budget.append(generated_text)
...
def main(llm, tokenizer, data_name, args):
    ...
    out_file_metrics = out_file.replace(".jsonl", f"_{args.prompt_type}_{args.thinking_token}_metrics.json")
    with open(out_file_metrics, "w") as f:
        json.dump(result_json, f, indent=4)
    result_json["time_use_in_second"] = time_use
    result_json["time_use_in_minite"] = f"{int(time_use // 60)}:{int(time_use % 60):02d}"
    result_json["word"] = args.thinking_token
    return result_json
...
if __name__ == "__main__":
    ...
    llm, tokenizer = load_model_and_tokenizer(args)
    prompt_words = load_jsonl(args.thinking_tokens_file_path)

    prompt_words = [
        entry for entry in prompt_words
        if re.fullmatch(r'[A-Za-z]+', entry.get('word', ''))
    ]

    output_file = f"{args.output_dir}/TTTS_results.json"

    aggregated_results = {}
    data_list = args.data_names.split(",")
    for data_name in data_list:
        aggregated_results[data_name] = {}
        for entry in prompt_words:
            word = entry["word"]
            args.thinking_token = word
            print(f"Evaluating dataset {data_name}, thinking token: {word}")
            result_json = main(llm, tokenizer, data_name, args)
            aggregated_results[data_name][word] = result_json
