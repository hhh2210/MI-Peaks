...
def parse_ground_truth(example: Dict[str, Any], data_name):
    # 如果样本中已经有 gt_cot 和 gt 字段，直接使用（这里不做区分大小写）
    if "gt_cot" in example and "gt" in example:
        if data_name.lower() in ["math", "minerva_math"]:
    ...
    elif data_name.lower() == "math-500":
        # 对于 MATH-500，使用 solution 作为链式推理，直接使用 answer 作为最终答案
        gt_cot = example["solution"]
        gt_ans = strip_string(example["answer"])
...
def parse_question(example, data_name):
    ...
    if data_name == "asdiv":
        ...
        for ch in "BCD":
            if f" {ch}) " in options:
                options = re.sub(f" {ch}\) ", f" ({ch}) ", options)
        question = f"{example['question'].strip()}\nAnswer Choices: {options}"
